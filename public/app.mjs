...
const paletteColors={classic:'#122b40',emerald:'#0c5e59',indigo:'#37376e',ocean:'#075b78',copper:'#7a4526'};
function applyAppearance(palette,mode){const chosenPalette=Object.hasOwn(paletteColors,palette)?palette:'classic',chosenMode=mode==='night'?'night':'day';document.documentElement.dataset.theme=chosenPalette;document.documentElement.dataset.mode=chosenMode;document.querySelector('meta[name="theme-color"]')?.setAttribute('content',chosenMode==='night'?'#101b2a':paletteColors[chosenPalette]);document.querySelectorAll('[data-theme-choice]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.themeChoice===chosenPalette)));document.querySelectorAll('[data-mode-choice]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.modeChoice===chosenMode)));try{localStorage.setItem('pos-reports-palette',chosenPalette);localStorage.setItem('pos-reports-mode',chosenMode)}catch{}}
...
document.querySelectorAll('[data-theme-choice]').forEach(button=>button.onclick=()=>applyAppearance(button.dataset.themeChoice,document.documentElement.dataset.mode));document.querySelectorAll('[data-mode-choice]').forEach(button=>button.onclick=()=>applyAppearance(document.documentElement.dataset.theme,button.dataset.modeChoice));
...
function isNumberColumn(index){return tab==='stock'?index>=2:index===2||index===3}
function columns(){return tab==='stock'?['Item','Company','Packs','Units / pack','Avg cost / pack','Trade / pack','Stored disc. %','Cost value']:['Code',tab==='payables'?'Supplier':'Customer','Stored balance (Rs)','Invoice total (Rs)','Last invoice','Check'];}
function cells(r,interactive=true){const name=interactive?`<button class="name-button" data-code="${esc(r.code)}">${esc(r.name)}</button>`:esc(r.name);
 ...
 return [esc(r.code),`${name}${r.phone?`<span class="subtext">${esc(r.phone)}</span>`:''}`,`<strong${r.balance<0?' class="negative"':''}>${fmt(r.balance)}</strong>`,`<strong>${fmt(r.invoiceTotal)}</strong><span class="subtext">${r.invoices?.length||0} unpaid invoice${(r.invoices?.length||0)===1?'':'s'}</span>`,date(r.lastDate),badge];
}
function tableHTML(rows,interactive=true){const c=columns();return `<thead><tr>${c.map((h,i)=>`<th class="${isNumberColumn(i)?'number':''}">${h}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${cells(r,interactive).map((v,i)=>`<td class="${isNumberColumn(i)?'number':''}">${v}</td>`).join('')}</tr>`).join('')}</tbody>`}
function render(){
 ...
 const total=filtered.reduce((a,r)=>a+(tab==='stock'?r.value||0:r.balance),0);$('filteredTotal').textContent=`Filtered ${tab==='stock'?'cost value':'stored balance'}: ${money(total)}`;
 $('reportNote').textContent=tab==='stock'?'Packs = stock units ÷ units per pack. Value uses stored average cost; not discounted again. Tap an item for purchase/sale discounts and raw fields.':`Stored ${tab==='payables'?'supplier':'customer'} balance is the POS ledger amount. Invoice total is the sum of that account’s unpaid invoice rows. “Review” means the figures differ; it does not change the stored balance.`;
...
function showDetails(code){
 const r=report[tab].find(x=>x.code===code);if(!r)return;$('detailTitle').textContent=r.name;
 if(tab==='stock'){
  const pairs=[['Item code',r.code],['Company',r.company],['Quantity (packs)',fmt(r.packs)],['Stock units',fmt(r.units)],['Units per pack',fmt(r.packSize,0)],['Packing',r.packing||'Not recorded'],['Average cost / pack',money(r.cost)],['Trade price / pack',money(r.trade)],['Retail price / pack',money(r.retail)],['Cost value',money(r.value)],['Stored discount (LDISC)',fmt(r.discount)+'%'],['Fixed discount field (FDISC)',fmt(r.fixedDiscount)],['Latest purchase discount',r.purchaseDiscount===null?'Not recorded':fmt(r.purchaseDiscount)+'%'],['Latest sale discount',r.saleDiscount===null?'Not recorded':fmt(r.saleDiscount)+'%'],['Last purchase',date(r.lastPurchase)],['Last sale',date(r.lastSale)],['Movement difference (units)',fmt(r.movementDiff)]];
  $('detailBody').innerHTML='<div class="detail-grid">'+pairs.map(([k,v])=>`<div><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('')+'</div><details><summary>Original backup fields</summary><div class="raw-grid">'+Object.entries({...r.raw,...r.stockFields}).map(([k,v])=>`<div><b>${esc(k)}</b>${esc(v===''?'—':v)}</div>`).join('')+'</div></details>';$('detailDialog').showModal();return;
 }
 const type=tab==='payables'?'Supplier':'Customer',invoiceCount=r.invoices?.length||0,totalAmount=(r.invoices||[]).reduce((total,item)=>total+item.amount,0),totalPaid=(r.invoices||[]).reduce((total,item)=>total+item.paid,0),difference=r.difference||0;
 const accountSection=`<section class="account-overview"><p class="eyebrow">${type.toUpperCase()} ACCOUNT</p><div class="account-meta"><span>Code <strong>${esc(r.code)}</strong></span>${r.phone?`<span>Phone <strong>${esc(r.phone)}</strong></span>`:''}${r.address?`<span class="wide">Address <strong>${esc(r.address)}</strong></span>`:''}</div><div class="account-stats"><div><small>POS ledger balance</small><strong${r.balance<0?' class="negative"':''}>${money(r.balance)}</strong><span>Current account balance</span></div><div class="emphasis"><small>Pending on invoices</small><strong>${money(r.invoiceTotal)}</strong><span>${invoiceCount} unpaid invoice${invoiceCount===1?'':'s'}</span></div><div><small>Account adjustment</small><strong${difference!==0?' class="negative"':''}>${money(difference)}</strong><span>${difference===0?'Matches invoice rows':'Ledger difference'}</span></div></div></section>`;
 const invoiceSection=`<section class="invoice-section"><div class="invoice-heading"><div><p class="eyebrow">PENDING INVOICE HISTORY</p><h3>Invoices awaiting payment</h3><p>${invoiceCount?`${invoiceCount} open invoice${invoiceCount===1?'':'s'} · latest ${date(r.lastDate)}`:'No open invoices found'}</p></div><strong>Pending: ${money(r.invoiceTotal)}</strong></div>${invoiceCount?`<div class="invoice-table-wrap"><table class="invoice-table"><thead><tr><th>Invoice</th><th>Date</th><th>Type</th><th class="number">Invoice amount</th><th class="number">Paid</th><th class="number">Pending</th></tr></thead><tbody>${r.invoices.map(item=>`<tr><td><strong>${esc(item.number)}</strong><span class="invoice-status">Pending</span></td><td>${esc(date(item.date))}</td><td><span class="source-tag">${esc(item.source||'Invoice')}</span></td><td class="number">${fmt(item.amount)}</td><td class="number">${fmt(item.paid)}</td><td class="number"><strong class="pending-money">${fmt(item.remaining)}</strong></td></tr>`).join('')}</tbody><tfoot><tr><th colspan="3">Invoice history total</th><th class="number">${fmt(totalAmount)}</th><th class="number">${fmt(totalPaid)}</th><th class="number">${fmt(r.invoiceTotal)}</th></tr></tfoot></table></div>`:'<p class="invoice-empty">No direct unpaid invoice rows were found for this account.</p>'}</section>`;
 $('detailBody').innerHTML=accountSection+invoiceSection+'';$('detailDialog').showModal();
}
function csvCell(v){let s=String(v??'');if(/^[=+@\-\t\r]/.test(s)&&typeof v!=='number')s="'"+s;return '"'+s.replaceAll('"','""')+'"'}
function exportCSV(){let headers,rows;if(tab==='stock'){headers=['Code','Item','Company','Packing','Packs','Stock units','Units per pack','Average cost','Trade price','Retail price','Stored discount LDISC','FDISC','Latest purchase discount','Latest sale discount','Cost value','Movement difference'];rows=filtered.map(r=>[r.code,r.name,r.company,r.packing,r.packs,r.units,r.packSize,r.cost,r.trade,r.retail,r.discount,r.fixedDiscount,r.purchaseDiscount,r.saleDiscount,r.value,r.movementDiff])}else{headers=['Code','Name','Stored balance Rs','Invoice total Rs','Unpaid invoice count','Phone','Address','Last invoice','Invoice calculation','Difference'];rows=filtered.map(r=>[r.code,r.name,r.balance,r.invoiceTotal,r.invoices?.length||0,r.phone,r.address,date(r.lastDate),r.calculated,r.difference])}const blob=new Blob(['\ufeff'+[headers,...rows].map(r=>r.map(csvCell).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`Nexus_Pulse_${tab}_${report.asOf}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),10000)}
function printReport(){$('printArea').innerHTML=`<h1>Nexus Pulse · ${labels[tab]}</h1><p>${esc(report.filename)} · Latest transaction ${date(report.asOf)} · ${filtered.length} filtered records</p><p>${esc($('filteredTotal').textContent)}</p><p>${esc($('reportNote').textContent)}</p><table>${tableHTML(filtered,false)}</table>`;window.print()}
$('importButton').onclick=()=>$('fileInput').click();$('fileInput').onchange=e=>importFile(e.target.files[0]);document.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>changeTab(b.dataset.tab));for(const id of ['search','filter','sort','order','company','minimum','alphabet'])$(id).addEventListener(id==='search'||id==='minimum'?'input':'change',()=>{page=1;render()});$('prev').onclick=()=>{page--;render()};$('next').onclick=()=>{page++;render()};$('tableBody').onclick=e=>{const b=e.target.closest('[data-code]');if(b)showDetails(b.dataset.code)};$('exportButton').onclick=exportCSV;$('printButton').onclick=printReport;$('helpButton').onclick=()=>$('helpDialog').showModal();document.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>b.closest('dialog').close());$('clearButton').onclick=async()=>{
...
async function init(){
 ...
 if(saved?.schema===2)setReport(saved,'Latest backup saved on this device · Reloading keeps your data.');else if(saved)notice('Import your backup once to load invoice details.');
 await takeShared();
}
