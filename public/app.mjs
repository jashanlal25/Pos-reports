...
function showDetails(code){const r=report[tab].find(x=>x.code===code);if(!r)return;$('detailTitle').textContent=r.name;
 const pairs=tab==='stock'?[['Item code',r.code],['Company',r.company],['Quantity (packs)',fmt(r.packs)],['Stock units',fmt(r.units)],['Units per pack',fmt(r.packSize,0)],['Packing',r.packing||'Not recorded'],['Average cost / pack',money(r.cost)],['Trade price / pack',money(r.trade)],['Retail price / pack',money(r.retail)],['Cost value',money(r.value)],['Stored discount (LDISC)',fmt(r.discount)+'%'],['Fixed discount field (FDISC)',fmt(r.fixedDiscount)],['Latest purchase discount',r.purchaseDiscount===null?'Not recorded':fmt(r.purchaseDiscount)+'%'],['Latest sale discount',r.saleDiscount===null?'Not recorded':fmt(r.saleDiscount)+'%'],['Last purchase',date(r.lastPurchase)],['Last sale',date(r.lastSale)],['Movement difference (units)',fmt(r.movementDiff)]]:[['Account code',r.code],['Stored balance',money(r.balance)],['Outstanding invoice total',money(r.invoiceTotal)],['Invoice calculation',r.calculated===null?'Unavailable':money(r.calculated)],['Difference',r.difference===null?'Unavailable':money(r.difference)],['Phone',r.phone||'Not recorded'],['Address',r.address||'Not recorded'],['Last invoice',date(r.lastDate)]];
 const invoiceSection=tab==='stock'?'':`<section class="invoice-section"><div class="invoice-heading"><div><p class="eyebrow">UNPAID INVOICES</p><h3>Outstanding invoices</h3></div><strong>Invoice total: ${money(r.invoiceTotal)}</strong></div>${r.invoices?.length?`<div class="invoice-table-wrap"><table class="invoice-table"><thead><tr><th>Invoice</th><th>Date</th><th class="number">Amount</th><th class="number">Paid</th><th class="number">Remaining</th></tr></thead><tbody>${r.invoices.map(item=>`<tr><td>${esc(item.number)}</td><td>${esc(date(item.date))}</td><td class="number">${fmt(item.amount)}</td><td class="number">${fmt(item.paid)}</td><td class="number"><strong>${fmt(item.remaining)}</strong></td></tr>`).join('')}</tbody><tfoot><tr><th colspan="4">Total remaining</th><th class="number">${fmt(r.invoiceTotal)}</th></tr></tfoot></table></div>`:'<p class="invoice-empty">No direct unpaid invoice rows were found for this account.</p>'}</section>`;
 $('detailBody').innerHTML='<div class="detail-grid">'+pairs.map(([k,v])=>`<div><small>${esc(k)}</small><strong>${esc(v)}</strong></div>`).join('')+'</div>'+invoiceSection+'<details><summary>Original backup fields</summary><div class="raw-grid">'+Object.entries({...r.raw,...r.stockFields}).map(([k,v])=>`<div><b>${esc(k)}</b>${esc(v===''?'—':v)}</div>`).join('')+'</div></details>';$('detailDialog').showModal();}
